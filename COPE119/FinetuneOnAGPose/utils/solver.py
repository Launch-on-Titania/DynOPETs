...
class Solver(gorilla.solver.BaseSolver):
    ...
    def solve(self):
        while self.epoch <= self.cfg.max_epoch:
            ...
            ckpt_path = os.path.join(
                self.cfg.ckpt_dir, 'epoch_' + str(self.epoch) + '_finetune_thres014.pt')
    ...
    def train(self):
        ...
        if self.dataset_name == "camera_real":
            ...
            iter_lenth = len(self.dataloaders["syn"])
        elif self.dataset_name == "REAL275" or self.dataset_name == "COPE119":
            data_iter = self.dataloaders["real"]
            iter_lenth = len(self.dataloaders["real"])
        else:
        ...
        for train_data in data_iter:
            ...
            if self.dataset_name == "COPE119" or self.dataset_name == "REAL275":
                method = getattr(self, f"step_COPE119")
            else:
    ...
    def step_COPE119(self, train_data, mode):
...
def test_func_COPE119(model, dataloader: torch.utils.data.DataLoader, save_path):
    model.eval()
    with tqdm(total=len(dataloader)) as t:
        ...
        for i, data in enumerate(dataloader):
            ...
            image_path = os.path.join(f"{Path(dataloader.dataset.images[i]).parent.parent}")
            std_result = {
                "pred_RTs":pred_RTs.detach().cpu().numpy(),
                "pred_scales": pred_scales.detach().cpu().numpy(),
                "image_id": i,
                "sequence_path": image_path,
                "class_ids": data['pred_class_ids'][0].numpy(),
                "pred_scores": data['pred_scores'][0].numpy(),
            ...
            rgb_path = Path(dataloader.dataset.images[i])
            # file_prefix = rgb_path.name[:4]
            # result_filename = f"{file_prefix}_result.pkl"
            # result_path = Path(save_path) / result_filename 
            # with open(result_path, 'wb') as f:
            #     cPickle.dump(result, f)


            t.set_description(
                "Test [{}/{}][{}]: ".format(i + 1, len(dataloader), num_instance)
            )
            t.update(1)
            
        pkl_path = os.path.join(save_path, f"{Path(dataloader.dataset.images[i]).parent.parent.name}.pkl.bz2")
        print(pkl_path)
        std_results.sort(key=lambda x: x['image_id'])
        with open(pkl_path, 'wb') as f:
            pickle.dump(std_results, f)
